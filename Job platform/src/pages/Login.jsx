...
export default function Login({setLoggedIn, setUsername, setUserRole}) {
        ...
        fetch("http://localhost:8081/auth/auth_token", {
            ...
            .then(r => {
                if (r.username === username) {
                    console.log('successfully authenticated:', r);
                    try {
                        ...
                        // Перенаправление на соответствующий главный экран
                        if (role === 'employer') {
                            navigate('/employer/dashboard'); // Перенаправление на главную работодателя
                        } else if (role === 'jobseeker') {
                            navigate('/jobseeker/dashboard'); // Перенаправление на главную соискателя
                        } else {
                            navigate('/'); // Перенаправление на главную, если роль неизвестна
                        }
                    } catch (error) {
                        console.error("Error decoding token:", error);
                        // Обработка ошибки декодирования токена, возможно, стоит показать сообщение пользователю
                    }
                } else {
    ...
    }, [username, password, navigate, setLoggedIn, setUsername, rememberMe, setUserRole]);
...
//         console.error("Authentication error:", error);
//       });
//   }, [username, password, navigate, setLoggedIn, setUsername, rememberMe]);
//
//   const handleSubmit = (e) => {
//     e.preventDefault();
//     if (validateForm()) {
